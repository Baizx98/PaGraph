...
def main(args):
    ...
    if args.sample:
        ...
        for rank in range(args.num_workers):
            subadj, _ = data.get_sub_train_graph(
                args.dataset, rank, args.num_workers)
            train_nid = data.get_sub_train_nid(
                args.dataset, rank, args.num_workers)
        ...
        print('Expected trainer#: {}. Start sampling at server end...'.format(
            args.num_workers))
        deliver = SampleDeliver(subgraph, sub_trainnid,
                                args.num_neighbors, hops, args.num_workers)
        deliver.async_sample(
            args.n_epochs, args.batch_size, one2all=args.one2all)
