    ...
    def __init__(self,
        ...
        if self.preprocess:  # NOTE: preprocess can only be enabled except lstm agg.
            ...
            if aggregator_type == 'lstm':
                self.reducer.append(
                    nn.LSTM(in_feats, in_feats, batch_first=True))
            self.layers.append(NodeUpdate(
                in_feats, n_hidden, activation, concat=skip_start))
        # hidden layers
        for i in range(1, n_layers):
            ...
            if aggregator_type == 'lstm':
                ...
                self.reducer.append(
                    nn.LSTM(n_hidden, n_hidden, batch_first=True))
            self.layers.append(NodeUpdate(
                n_hidden, n_hidden, activation, concat=skip_start))
        # output layers
        if aggregator_type == 'lstm':
            self.reducer.append(
                nn.LSTM(2 * n_hidden, 2 * n_hidden, batch_first=True))
